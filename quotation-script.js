...
const categories = [
    { id: "CecoCeco", label: "ArtMorph", image: "Artmorph.png", note: "Creative and architectural display" },
    { id: "Fixed Install", label: "Fix Install", image: "Fix Install.png", note: "Permanent installation display" },
    { id: "Rental-Indoor", label: "Rental-Indoor", image: "Rental-Indoor.png", source: "Rental", waterproof: "Indoor", note: "Indoor touring and staging display" },
    { id: "Rental-Outdoor", label: "Rental-Outdoor", image: "Rental-Outdoor.png", source: "Rental", waterproof: "Outdoor", note: "Outdoor touring and staging display" }
];
...
// Route direction rules per product category (matching mapping.html)
function applyRouteDirectionRules() {
    ...
    if (source === "Rental") {
        // Rental Indoor & Outdoor: vertical only, locked
        defaultValue = "vertical"; locked = true;
    } else if (source === "Fixed Install" || cat.id === "Fixed Install") {
        // Fix Install: default horizontal, can switch
        defaultValue = "horizontal"; locked = false;
    } else if (cat.id === "CecoCeco") {
        // ArtMorph: default vertical, can switch
        defaultValue = "vertical"; locked = false;
    }
    // Only change if currently invalid or first load
    if (locked || !["vertical", "horizontal"].includes(select.value)) {
        select.value = defaultValue;
    }
    select.disabled = locked;
    // Update label to indicate lock status
    const label = select.closest(".field")?.querySelector("label");
    if (label) label.textContent = locked ? "Wiring Direction (Locked)" : "Wiring Direction";
}
...
function productImage(model = state.model) {
    return state.productImages?.[model]?.file || `product%20picture/${safeModelFile(model)}.png`;
}
...
function appendixPage(title, cards, svg) {
    return `<section class="a4-page wiring-appendix">
        <div class="letterhead-header"><img src="assets/roe-letterhead-header.png" alt="ROE Visual"></div>
...
async function buildQuotePdf() {
    ...
    const headerImage = await loadPdfImage("assets/roe-letterhead-a4.png").catch(() => null);
...
Promise.all([
    ...
    fetch("product%20picture/image-map.json").then(response => response.json()).catch(() => ({}))
])
